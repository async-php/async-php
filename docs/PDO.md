...
该仓库提供一个用户态的 `PDO`/`PDOStatement` 兼容层（位于 `src-php/Async/PDO`），底层通过扩展暴露的 `Async\Kernel\PDO\MySql` / `Async\Kernel\PDO\PgSql` 执行 SQL。

## 使用前提

- 必须加载 async-php 扩展（需要 `run()` / `go()` 等函数存在）。
- 需要禁用 PHP 原生 `pdo` 扩展，否则无法声明全局类 `PDO`/`PDOStatement`/`PDOException`。
